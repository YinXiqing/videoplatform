    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">观看历史</h1>
          {history.length > 0 && (
            ...
            {history.map(({ watched_at, video }) => (
              <Link key={`${video.id}-${watched_at}`} href={`/video/${video.id}`}
                ...
                <div className="relative w-40 h-24 bg-gray-900 rounded-lg overflow-hidden flex-shrink-0">
                  {video.cover_image
                    ? <img src={video.is_scraped && video.cover_image.startsWith('http') ? video.cover_image : `/api/video/cover/${video.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{new Date(watched_at).toLocaleString()}</p>
                </div>
              </Link>
            ))}
          </div>
