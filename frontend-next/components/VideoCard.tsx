		...
		>
			<Link href={`/video/${video.id}`}>
				<div className="relative aspect-video overflow-hidden bg-gray-900">
					...
					{(video.duration ?? 0) > 0 && (
						<div
							className={`absolute bottom-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded z-20 ${isHovered && videoLoaded ? "opacity-0" : "opacity-100"}`}
						>
			...
			<div className="p-4">
				...
				<div className="flex items-center text-xs text-gray-400 dark:text-gray-500 space-x-2">
					<span>
						{formatViews?.(video.view_count) ?? video.view_count} 次观看
					</span>
					<span>•</span>
